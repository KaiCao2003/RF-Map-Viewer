...
import type { DatasetMeta, Matrix } from "./types";
import {
...
const meta: DatasetMeta = {
  id: "fixture",
  name: "fixture.json",
  sourcePath: "/data/rfmapping/fixture.json",
  shape: [1, 2, 3, 4],
  unitPool: [17],
  xPositions: [-10, 0, 10],
  yPositions: [5, 15],
  timeBinEdges: [-0.1, -0.05, 0, 0.05, 0.1],
  presentationCounts: [[2, 2, 2], [4, 4, 4]],
...
describe("RF display math", () => {
  ...
  it("normalizes count values by presentations and duration", () => {
    ...
    expect(responseMatrix(counts, meta, 2, 3, "Mean firing rate (Hz)")[0]).toEqual([25, 0, 10]);
  });
  ...
  it("excludes unexposed source pixels and pools observations before normalization", () => {
    ...
    expect(prepareResponseMatrix(pooledCounts, pooledMeta, [0, 0], "Spikes / presentation", 1, 1, false, 0).matrix).toEqual([[1]]);
  ...
  it("smooths pooled counts and exposures separately", () => {
    ...
    expect(prepared.matrix[0][0]).toBeCloseTo(10.398009950248756);
    expect(prepared.matrix[0][1]).toBeCloseTo(11.568627450980392);
