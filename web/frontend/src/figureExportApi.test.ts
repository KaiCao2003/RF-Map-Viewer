...
describe("figure export API", () => {
  ...
  it("posts companion-aware preview plans and exposes placeholder headers", async () => {
    ...
    const request: FigurePreviewRequest = {
      ...
      hdPath: "/data/rfmapping/session/tuning_curves.json",
    };
  ...
  it("normalizes final export status and manifest placeholders", async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse({
      format: "pdf",
      path: "/data/rfmapping/session/report.pdf",
      pageCount: 1,
      bytes: 2048,
      overwritten: false,
      manifest: {
        ...
        format: "pdf",
        order: "unit-major",
        source: "/data/rfmapping/session/rf.json",
        pages: [{
          outputIndex: 0,
          clusterId: 7,
          unitIndex: 1,
    ...
    expect(result.path).toBe("/data/rfmapping/session/report.pdf");
