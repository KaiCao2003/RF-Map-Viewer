...
Build and test an immutable release under /srv/rfmapping/releases.
...
for required_path in \
    ...
    Utils/__init__.py \
    Utils/figure_export.py \
    rfmapping_web/app.py \
    web/package.json \
    web/package-lock.json \
    deploy/rfmapping-web.service \
    deploy/nginx-rfmapping-location.conf \
    tests/test_rfmapping_web.py \
    tests/test_access_gate.py \
    tests/test_figure_export.py; do
...
if [[ "${install_deps}" == true ]]; then
    "${RFMAPPING_PIP}" install --requirement "${SOURCE_ROOT}/requirements-web.txt"
fi
...
rsync -a --exclude '__pycache__/' --exclude '*.pyc' \
    "${SOURCE_ROOT}/rfmapping_web/" "${staging_dir}/rfmapping_web/"
install -d -m 0755 "${staging_dir}/Utils"
install -m 0644 \
    "${SOURCE_ROOT}/Utils/__init__.py" \
    "${SOURCE_ROOT}/Utils/figure_export.py" \
    "${staging_dir}/Utils/"
rsync -a --exclude 'node_modules/' --exclude 'dist/' \
    "${SOURCE_ROOT}/web/" "${staging_dir}/web/"
rsync -a --exclude '__pycache__/' --exclude '*.pyc' \
    "${SOURCE_ROOT}/tests/" "${staging_dir}/tests/"
rsync -a "${SOURCE_ROOT}/deploy/" "${staging_dir}/deploy/"
install -m 0644 "${SOURCE_ROOT}/requirements-web.txt" "${staging_dir}/requirements-web.txt"
