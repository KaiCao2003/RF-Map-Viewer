# RFmapping Web deployment on rfmapping_remote

The private-network URL is
`http://rfmapping_remote.fsm.northwestern.edu/rfmapping/` (or the same `/rfmapping/`
...
RF datasets and companion files under `/data/rfmapping` are read-only through
the viewer API. Caches live under `/srv/rfmapping/cache`, and legacy
CSV/image saves live under `/srv/rfmapping/exports`. The figure
composer has a separate, explicit publication boundary,
`RFMAPPING_FIGURE_EXPORT_ROOT` (default `/data/rfmapping`), so a user can choose
...
- Host: Ubuntu 24.04 on `rfmapping_remote`; Nginx 1.24 and systemd 255 are active.
- Runtime: `/home/rfmapping/.virtualenvs/rfmapping`; a lingering user systemd manager
  runs the service as `rfmapping`, so releases and rollbacks need no sudo.
- Storage: `/srv/rfmapping-storage` had about 3.0 TB free during implementation.
- Privileges: `rfmapping` is in the `sudo` group but does not have passwordless sudo.
  Only the optional Nginx installation/reload needs an interactive
  `ssh -t rfmapping_remote` session and `sudo -v`. Scripts never prompt for a password.
- Port 3005 is reserved for the loopback-only RFmapping service; ports
  3000-3004 are assigned to existing rfmapping_remote tools.
...
`198.51.100.0/24`, `192.0.2.0/24`, and `203.0.113.0/24`; all other forwarded
...
The Nginx snippet permits loopback plus `198.51.100.0/24`, `192.0.2.0/24`,
and `203.0.113.0/24` and rejects other clients. The Web app accepts small JSON
control requests, so this location needs no large-request proxy tuning.

## Release and rollback

For each release, sync the approved checkout to `~/Developer/rfmapping` and run:
...
3. Confirm the user service and direct routing:
   ...
   The health response must report `1.9.0-web`.
...
4. If the optional Nginx include is active, validate it separately:
   ...
   ./deploy/validate_real_data.sh \
     ...
     --host-header rfmapping_remote.fsm.northwestern.edu
