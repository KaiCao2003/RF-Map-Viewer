...
configuration is never overwritten.
  ...
  --install-deps  Install requirements-web.txt into ~/.virtualenvs/rfmapping.
...
[[ -f "${SOURCE_ROOT}/requirements-web.txt" ]] || deploy_die "requirements-web.txt is missing from ${SOURCE_ROOT}"
...
if [[ -e "${ENV_TARGET}" ]]; then
    ...
    if ! grep -q '^RFMAPPING_OUTPUT_ROOT=' "${ENV_TARGET}"; then
        printf '\n# Persistent server-side GUI exports on rfmapping_remote local SSD.\nRFMAPPING_OUTPUT_ROOT=%s\n' \
