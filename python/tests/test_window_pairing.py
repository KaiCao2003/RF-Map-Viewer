...
class WindowPairingTests(unittest.TestCase):
    ...
    def test_control_normalization_keeps_selected_cell_in_current_groups(self) -> None:
        viewer = SimpleNamespace(
            ...
            range_start_ms_var=Variable("0"),
            range_end_ms_var=Variable("10"),
            subtract_start_ms_var=Variable("0"),
            subtract_end_ms_var=Variable("10"),
            data=SimpleNamespace(time_bin_edges=[0.0, 0.01]),
            _timeline_range_anchor=None,
            selected_cell=(2, 3, 4, 5),
            _time_groups=mock.Mock(return_value=[(0, 0)]),
            _source_bins_for_time_controls=mock.Mock(return_value=(0, 0)),
