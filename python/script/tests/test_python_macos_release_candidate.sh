...
python3 - "$METADATA_AUDITOR" "$HOOK" <<'PY' \
...
) || fail_test "canonical Full release metadata is incomplete or unexpected"
pass_test "canonical metadata identifies Python 1.9.2 Full build 10902"
...
assert pyproject["project"]["version"] == "1.9.2"
...
PY
pass_test "package metadata restores the pinned TkDND runtime"
...
for marker in \
  ...
  'Add :UTExportedTypeDeclarations:2:UTTypeIdentifier string org.local.rfmapping.probe' \
...
pass_test "Full bundle restores smoke tests, resources, platform metadata, signing, and clean archives"

# shellcheck source=../build_python_macos_app.sh
source "$BUILD_SCRIPT"
[[ "$ARCHIVE_NAME" == "RF_Map_Viewer-python-1.9.2-full-macos-arm64.zip" ]] \
  || fail_test "archive name does not preserve version and Full flavor"
[[ "$CHECKSUM_NAME" == "SHA256SUMS-python-1.9.2-full.txt" ]] \
  || fail_test "checksum name does not preserve version and Full flavor"
pass_test "Full artifacts are independently named from the frozen Minimal release"
...
printf '%s\n' \
  ...
  >"$AUDIT_FIXTURE/rfmapping_gui.py"
python3 "$METADATA_AUDITOR" "$AUDIT_FIXTURE" 1.9.2 Full >/dev/null \
  || fail_test "metadata auditor rejected a matching Full release"
printf '%s\n' \
  'APP_VERSION = "1.9.0"' \
  'APP_EDITION = "Minimal"' \
  'DND_SMOKE_ARGUMENT = "--self-test-dnd"' \
  >"$AUDIT_FIXTURE/rfmapping_gui.py"
if python3 "$METADATA_AUDITOR" "$AUDIT_FIXTURE" 1.9.2 Full >/dev/null 2>&1; then
  fail_test "metadata auditor accepted stale Minimal source as Full 1.9.2"
fi
...
with zipfile.ZipFile(root / "clean.zip", "w") as archive:
    archive.writestr("RF Map Viewer.app/Contents/Info.plist", "clean")
with zipfile.ZipFile(root / "appledouble.zip", "w") as archive:
    archive.writestr("RF Map Viewer.app/Contents/Info.plist", "clean")
    archive.writestr("__MACOSX/RF Map Viewer.app/Contents/._Info.plist", "bad")
PY
(
  # shellcheck source=../build_python_macos_app.sh
  source "$BUILD_SCRIPT"
...
python3 - "$SMOKE_JSON" <<'PY' \
  || fail_test "synthetic frozen-executable smoke JSON is malformed"
import json
import pathlib
import sys

payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["unitsSpikeCountsSize"] == [1, 1, 2, 3]
assert payload["unitPool"] == [42]
assert len(payload["timeBinEdges"]) == 4
assert "synthetic" in payload["fixture"]
PY
pass_test "release smoke fixture is minimal, structural, and explicitly synthetic"
