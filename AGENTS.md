...
- Run project code only on the remote host reached with `ssh rfmapping_remote`.
...
- RF JSON, HD JSON, and probe CSV files are read-only inputs. Treat files under
  `/data/rfmapping/recordings` as authoritative.
- Production Web deployment remains under `/srv/rfmapping`; moving
