...
- Run this project only on the remote host reached with `ssh rfmapping_remote`.
...
- Do not run the project from the local checkout; run commands through SSH, for example:
  ...
  ```sh
  ssh rfmapping_remote 'cd ~/Developer/rfmapping && ~/.virtualenvs/rfmapping/bin/python <script>.py'
  ```
...
- Original MATLAB data-generation sources live on the Linux remote host under
  `/srv/rfmapping-storage/Matlab` (not in the local checkout). When investigating JSON
  generation issues, inspect those `.m` files through `ssh rfmapping_remote`; the
...
- The active pipeline is:
  ...
  -> /srv/rfmapping-storage/Matlab/RFmapping.m
  -> ~/Developer/rfmapping GUI
  ```

- Treat remote raw/session data as source of truth. For timing or JSON-generation
  questions, inspect the relevant files under `/data/rfmapping/recordings`
  and `/srv/rfmapping-storage/Matlab`; do not rely on stale JSON copies in the GUI repo.
