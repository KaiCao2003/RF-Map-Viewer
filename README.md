...
`/data/rfmapping`): the destination must already exist below that root, may not
...
Follow `AGENTS.md`: run project code only on `ssh rfmapping_remote` with
`~/.virtualenvs/rfmapping`. MATLAB files are not runtime dependencies. Typical
Python/Web gates are:

```sh
ssh rfmapping_remote 'cd ~/Developer/rfmapping && \
  PYTHONDONTWRITEBYTECODE=1 ~/.virtualenvs/rfmapping/bin/python -m pytest -q'

ssh rfmapping_remote 'cd ~/Developer/rfmapping/web && npm ci --no-audit --no-fund && \
